	...
	var util = Pikaday.util,
		...
		Pikarange = function (options) {
			...
			function setStartRange(d, temporary) {
				...
				if (
					endPicker._date &&
					(endPicker._date < minEndDate || endPicker._date > maxEndDate)
				) {
			...
			function handleFocus(event) {
				...
				if (containsElement(startPicker.el, event.target)) {
					a = startPicker;
					b = endPicker;
				} else if (containsElement(endPicker.el, event.target)) {
					a = endPicker;
					b = startPicker;
				}
				if (a && a !== activePicker) {
					...
					addClass(a._options.field, 'is-focused');
					removeClass(b._options.field, 'is-focused');
				}
				if (!a) {
					removeClass(startPicker._options.field, 'is-focused');
					removeClass(endPicker._options.field, 'is-focused');
				}
			}
			...
			function getPickerOver(picker) {
				return function handlePickerOver(event) {
					...
					var date = new Date(
						targetEl.getAttribute('data-pika-year'),
						targetEl.getAttribute('data-pika-month'),
						targetEl.getAttribute('data-pika-day')
					);
